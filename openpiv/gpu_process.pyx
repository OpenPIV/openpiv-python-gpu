...
def WiDIM( np.ndarray[DTYPEi_t, ndim=2] frame_a,
    ...
    >>> x,y,u,v, mask = openpiv.gpu_process.WiDIM( frame_a, frame_b, mark, min_window_size=16, overlap_ratio=0.25,
    coarse_factor=2, dt=0.02, validation_method='mean_velocity', trust_1st_iter=1, validation_iter=2, tolerance=0.7, nb_iter_max=4, sig2noise_method='peak2peak')
...
def gpu_interpolate_surroundings(d_F, v_list, Nrow, Ncol, W, Overlap, K, dat):
    ...
    if bottom_ind.size > 0:
        ...
        # Get values to compute interpolation
        d_y1, d_low_y = gpu_array_index(d_F[K, int(Nrow[K]-1), :, 1].copy(), d_low_y, np.float32, ReturnList = True)
        d_y2, d_high_y = gpu_array_index(d_F[K, int(Nrow[K]-1), :, 1].copy(), d_high_y, np.float32, ReturnList = True)
        d_y, d_bottom_ind = gpu_array_index(d_F[K+1, int(Nrow[K+1]-1), :, 1].copy(), d_bottom_ind, np.float32, ReturnList = True)

        # return the values of the function surrounding the validation point
        d_f1 = gpu_array_index(d_F[K, int(Nrow[K]-1), :, dat].copy(), d_low_y, np.float32)
        d_f2 = gpu_array_index(d_F[K, int(Nrow[K]-1), :, dat].copy(), d_high_y, np.float32)

        # do interpolation
        d_bottom_linear = linear_interp_gpu(d_y1, d_y2, d_y, d_f1, d_f2)

        # Update values. Return a tmp array and destroy after to avoid GPU memory leak.
        d_tmp_bl = gpu_index_update(d_F[K+1, int(Nrow[K+1]-1), :Ncol[K+1], dat].copy(), d_bottom_linear, d_bottom_ind)
        d_F[K+1, int(Nrow[K+1]-1), :Ncol[K+1], dat] = d_tmp_bl
    ...
    if right_ind.size > 0:
        ...
        # Get values to compute interpolation
        d_x1, d_low_x = gpu_array_index(d_F[K, :, int(Ncol[K]-1), 0].copy(), d_low_x, np.float32, ReturnList = True)
        d_x2, d_high_x = gpu_array_index(d_F[K, :, int(Ncol[K]-1), 0].copy(), d_high_x, np.float32, ReturnList = True)
        d_x, d_right_ind = gpu_array_index(d_F[K+1, :, int(Ncol[K+1]-1), 0].copy(), d_right_ind, np.float32, ReturnList = True)

        # return the values of the function surrounding the validation point
        d_f1 = gpu_array_index(d_F[K, :, int(Ncol[K]-1), dat].copy(), d_low_x, np.float32)
        d_f2 = gpu_array_index(d_F[K, :, int(Ncol[K]-1), dat].copy(), d_high_x, np.float32)

        # do interpolation
        d_right_linear = linear_interp_gpu(d_x1, d_x2, d_x, d_f1, d_f2)

        # Update values. Return a tmp array and destroy after to avoid GPU memory leak.
        d_tmp_rl = gpu_index_update(d_F[K+1, :Nrow[K+1], int(Ncol[K+1]-1), dat].copy(), d_right_linear, d_right_ind)
        d_F[K+1, :Nrow[K+1], int(Ncol[K+1]-1), dat] = d_tmp_rl
...
def gpu_divergence(d_u, d_v, w, Nrow, Ncol):
    ...
    d_div[0, int(Ncol-1)] = (d_u[1, Ncol-1] - d_u[0, Ncol-1])/w - (d_v[0, Ncol-1] - d_v[0, Ncol-2])/w
    d_div[int(Nrow-1), 0] = (d_u[Nrow-1, 0] - d_u[Nrow-2, 0])/w - (d_v[Nrow-1, 1] - d_v[Nrow-1, 0])/w
...
def gpu_round(d_src, ReturnInput=False):
    ...
    if ReturnInput is False:
        # free some gpu memory
        d_src.gpudata.free()
        return d_dst
    elif ReturnInput is True:
        return d_dst, d_src
    else:
        raise ValueError("ReturnInput is currently {}. Must be of type boolean".format(ReturnInput))
