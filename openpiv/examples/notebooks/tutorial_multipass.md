...
```python
x, y, u, v, mask = process.gpu_piv_def(frame_a.astype(np.int32), frame_b.astype(np.int32),
                                       ones_like(frame_a).astype(np.int32), min_window_size=32, overlap_ratio=0.25,
                                       nb_refinement_iter=0, dt=0.1, validation_method='mean_velocity',
                                       trust_1st_iter=0, nb_validation_iter=0, mean_tol=0.7, nb_iter_max=1,
...
```python
x, y, u, v, mask = process.gpu_piv_def(frame_a.astype(np.int32), frame_b.astype(np.int32),
                                       ones_like(frame_a).astype(np.int32), min_window_size=16, overlap_ratio=0.25,
                                       nb_refinement_iter=2, dt=0.1, validation_method='mean_velocity',
                                       trust_1st_iter=1, nb_validation_iter=2, mean_tol=0.7, nb_iter_max=4,
                                       sig2noise_method='peak2peak')
```
