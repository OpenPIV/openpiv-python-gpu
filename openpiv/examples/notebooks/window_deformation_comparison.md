...
with warnings.catch_warnings():
    ...
    x, y, u, v, mask = process.gpu_piv_def(frame_a.astype(np.int32), frame_b.astype(np.int32), mark, min_window_size=16,
                                           overlap_ratio=0.0, nb_refinement_iter=2, dt=1,
                                           validation_method='mean_velocity', trust_1st_iter=1, nb_validation_iter=1,
                                           mean_tol=0.7, nb_iter_max=3, sig2noise_method='peak2peak')
...
tools.display_vector_field('2image_00.txt', widim=True, on_img=False,
                           image_name=os.path.join(path, '../test2/2image_00.tif'), window_size=16,
                           scaling_factor=scaling_factor, scale=200, width=0.001)
